...
function startWatcher() {
    console.log("%c Searching for subtitles... (checking every second)", "color: cyan;");

    const subtitleInterval = setInterval(async () => {
        // Retrieve all network resources loaded in the current session
        const resources = performance.getEntriesByType('resource');
        
        // Look for the Udemy VTT subtitle file in the resource list
        const vttResource = resources.find(r => r.name.includes('vtt-c.udemycdn.com'));

        if (vttResource) {
            console.log("%c Subtitle file found! Starting translation process...", "color: green; font-weight: bold;");
            
            // Stop the interval to prevent multiple injections
            clearInterval(subtitleInterval);
            
            // Execute the main translator function
            try {
            ...
            // console.log("Still waiting for VTT resource...");
        }
